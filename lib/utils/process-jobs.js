  ...
  function jobQueueFilling(name) {
    ...
    self._findAndLockNextJob(name, definitions[name], (err, job) => {
      ...
      if (job) {
        ...
        // Gregson's fix for concurrency
        // See https://github.com/mgregson/agenda/commit/5d0278ccef85e9b12a22319ba4db4de401f5bcd0
        var jobOccurrenceCount = 0;
        jobQueue.forEach(function(queued_job){
          if(queued_job.name === name) jobOccurrenceCount++;
        });
        if(jobQueue.length < 2 * self._maxConcurrency && jobOccurrenceCount < definitions[name].concurrency) {
